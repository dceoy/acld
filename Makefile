.DEFAULT_GOAL := help

VARIANT ?= base
CONTAINERFILE ?= Containerfile.$(VARIANT)
IMAGE ?= ghcr.io/dceoy/acld-$(VARIANT):latest
NAME ?= acld-$(VARIANT)
HOST_IP ?= 127.0.0.1
PORT ?= 6080
ORACLE_PORT ?= 9473
CPUS ?= 4
MEMORY ?= 4G
VNC_GEOMETRY ?= 1440x900
VNC_DEPTH ?= 24
VNC_PASSWORD ?=
HOME_VOLUME ?=
WORKSPACE_DIR ?=
MIN_MACOS_MAJOR ?= 26

export VARIANT CONTAINERFILE IMAGE NAME HOST_IP PORT ORACLE_PORT CPUS MEMORY VNC_GEOMETRY VNC_DEPTH VNC_PASSWORD HOME_VOLUME WORKSPACE_DIR MIN_MACOS_MAJOR

.PHONY: help check variants pull build up down status clean shell

help check variants pull build up down status clean shell:
	@./acld.sh $@
